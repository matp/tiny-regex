...
void printNode(FILE *file, RegexNode *node, int depth) {
  switch (node->type) {
  ...
  case REGEX_NODE_TYPE_CHARACTER:
    fprintf(file, isprint(node->ch)
      ? "character('%c')"
      : "character(%02x)", node->ch);
    break;
...
void printCharacterClass(FILE *file,
  const RegexProgramInstruction *instruction) {
  for (int ch = 0, to; ch < UCHAR_MAX; ++ch) {
    if (regexCharacterClassContains(instruction->klass, ch)) {
      fprintf(file, isprint(ch) ? "%c" : "%02x", ch);
      for (to = ch + 1;
        regexCharacterClassContains(instruction->klass, to); ++to);
      if (to > ch + 2) {
...
void printInstruction(FILE *file, const RegexProgram *program,
  const RegexProgramInstruction *instruction) {
...
void printProgram(FILE *file, const RegexProgram *program) {
...
int main(int argc, char *argv[]) {
  ...
  program = regexCompileNode(node);
  regexParseFree(node);
  if (program)
    printProgram(stdout, program);
  else {
    fprintf(stderr, "%s: regexCompileNode() failed\n", argv[0]);
    return EXIT_FAILURE;
  }
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    if (regexProgramRun(program, argv[i], matches, 20) > 0) {
      ...
      for (int j = 0; j <= nmatches; j += 2) {
        ...
        if (matches[j] && matches[j + 1]) {
          printf("\"%.*s\"(%d,%d)",
            ...
            (int)(matches[j] - argv[i]), (int)(matches[j + 1] - argv[i]));
        } else {
          printf("(NULL,NULL)");
        }
      }
